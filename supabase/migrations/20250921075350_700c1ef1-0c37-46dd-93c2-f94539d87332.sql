-- Fix ambiguous column reference in create_or_get_conversation function
...
BEGIN
  ...
  -- Create participant records for both users - fix ambiguous column reference
  INSERT INTO conversation_participants (conversation_id, user_id, created_at) 
  VALUES 
    (conv_id, user_a, now()),
    (conv_id, user_b, now())
  ON CONFLICT (conversation_participants.conversation_id, conversation_participants.user_id) DO NOTHING;
