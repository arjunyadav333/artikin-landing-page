...
CREATE INDEX IF NOT EXISTS idx_posts_created_user_optimized 
ON posts (created_at DESC, user_id);

CREATE INDEX IF NOT EXISTS idx_profiles_user_id_optimized 
ON profiles (user_id);

CREATE INDEX IF NOT EXISTS idx_likes_user_post_optimized 
ON likes (user_id, post_id);

CREATE INDEX IF NOT EXISTS idx_connections_follower_optimized 
...
CREATE INDEX IF NOT EXISTS idx_messages_conversation_created_optimized 
ON messages (conversation_id, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_message_receipts_user_status_optimized 
ON message_receipts (user_id, status);

-- Ultra-fast user interaction lookups
CREATE INDEX IF NOT EXISTS idx_saves_user_post_optimized 
ON saves (user_id);

-- Optimize conversation participants for real-time features
CREATE INDEX IF NOT EXISTS idx_conversation_participants_user_optimized 
ON conversation_participants (user_id, conversation_id);

-- Comments performance optimization
CREATE INDEX IF NOT EXISTS idx_comments_post_created_optimized 
