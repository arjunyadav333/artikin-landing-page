-- Phase 1.2: Implement Proper User Roles System (Fixed)

-- Create enum for app roles (check if exists first)
DO $$ 
BEGIN
...
BEGIN
  ...
  -- Insert into user_roles if role is determined
  IF determined_role IS NOT NULL THEN
    INSERT INTO public.user_roles (user_id, role)
    VALUES (NEW.id, determined_role)
    ON CONFLICT (user_id, role) DO NOTHING;
  END IF;
